...
describe SQL::Clauses do
  ...
  include_examples "Clause", :left_join, :"LEFT JOIN", :users
  include_examples "Clause", :right_join, :"RIGHT JOIN", :users
  include_examples "Clause", :full_join, :"FULL JOIN", :users
  include_examples "Clause", :on, :ON, proc { id == 1 }
  include_examples "Clause", :union, :UNION, proc { select(:*).from(:users) }
